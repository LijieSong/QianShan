...
public class WheelRecycle {
	 ...
	 * @param firstItem the number of first item_grideview in layout
	 * @param range the range of current wheel items 
	 * @return the new value of first item_grideview number
	 */
	public int recycleItems(LinearLayout layout, int firstItem, ItemsRange range) {
		...
		for (int i = 0; i < layout.getChildCount();) {
			if (!range.contains(index)) {
				...
				if (i == 0) { // first item_grideview
					firstItem++;
				}
			} else {
				i++; // go to next item_grideview
			}
			index++;
		}
		return firstItem;
	...
	/**
	 * Gets empty item_grideview view
	...
	/**
	 * Adds view to cache. Determines view type (item_grideview view or empty one) by index.
	 * @param view the view to be cached
