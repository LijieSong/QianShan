 ...
 */
public class DoctorListAdapter extends RecyclerView.Adapter implements View.OnClickListener {
    private Context context;
    private List<OneKeySaveBean> list;
    private OnRecyclerViewItemClickListener mOnItemClickListener = null;

    public DoctorListAdapter(Context context, String[] names, String[] mobiles) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_doctor_list, null);
...
//        holde.cimg_tou_xiang.setBackgroundDrawable(CommonUtil.getDrawable(R.drawable.logo));
        ...
        holde.imgbtn_call.setVisibility(View.GONE);
        //按钮
        holde.imgbtn_call.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(context, VideoCallActivity.class);
                intent.putExtra("userId", list.get(position).getMobile());
                intent.putExtra("isComingCall", false);
                PreferencesUtils.putString(context.getApplicationContext(),"username", list.get(position).getMobile());
                if (ActivityCompat.checkSelfPermission(context, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions((Activity)context,new String[]{Manifest.permission.CAMERA,Manifest.permission_group.CAMERA,Manifest.permission.CALL_PHONE },22);
                } else {
